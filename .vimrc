...
set shiftwidth=4
set relativenumber
set noshowmode "hide the mode on left corner
...
set makeprg=g++\ -Wall\ -Wshadow\ -O2\ -Wno-unused-result\ -std=c++14\ %\ -o\ %:r
autocmd filetype cpp map <F9> :w <bar> make <CR> <CR>:let g:is_open = 0<CR> : call ToggleQuickFix()<CR>
autocmd filetype cpp map <F9> :w <bar> make <CR> <CR>:let g:is_open = 0 <CR> :call ToggleQuickFix()<CR>
map <F3> :./%<
map <F8> :!gnome-terminal -e "/bin/bash -c './%<;  echo;  echo Press ENTER to continue; read line;exit; exec /bin/bash -i'"<CR> <CR>
